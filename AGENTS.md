...
- **Cloudflare Images transformations — ATSAKOTOS šiam projektui (savininko sprendimas 2026-09-09)** — naujų `cf.image` taškų nekurti, jokių išimčių (anksčiau buvęs „savininkui patvirtinus" kelias panaikintas). Legacy du taškai tik palaikomi, neplėstini: `/api/mods/:id/thumbnail/img` (w ∈ {64,96,128}) ir `/api/img/proxy` (w ∈ {384,768,960,1200,1600,1920}); neleistinas `w` → 302 į kanoninį. Jų šalinimas — tik savininko sprendimu su pakaitalu (~2.4 MB mobilioji regresija be jo; kryptys ir detalių `docs/COST_GUARDRAILS.md`). Senasis Pages projektas sunaikintas 2026-09-09 — jo neatkurti.
- **KV read amplifikacija** — naujas funkcionalumas NEmato per-mod `kv.get` loop'ų sąrašams (dėl to buvo 1.45M reads/dieną); naudoti agreguotus bundle raktus (pvz. `cache:bundle:modfields`). Ribos ir stebėjimas — `docs/COST_GUARDRAILS.md`.
